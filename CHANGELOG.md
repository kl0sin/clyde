...
- **Worktree sessions are labelled.** A session working inside a git worktree gets a green badge with the worktree's name, instead of showing an opaque `.claude/worktrees/…` path. The badge drops the moment the session leaves the worktree. Clyde reads this off the session's own working directory and deliberately does *not* subscribe to Claude Code's `WorktreeCreate` event: that event asks its subscriber to *create* the worktree and hand the path back, so an advisory listener like Clyde would have broken the `EnterWorktree` tool outright. If you ran a development build that registered it, Clyde now notices the leftover registration on launch and clears it for you.
