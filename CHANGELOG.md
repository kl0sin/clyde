...
- **The shortcut says it needs two permissions, and opens both.** ⌃⌘C needs accessibility *and* input monitoring, and macOS puts them in different panes. The advisory named only the one that was missing, so granting it produced a second warning about a permission nobody had mentioned — and the input-monitoring advisory had no button at all, leaving the user to find that pane themselves. Both advisories now name both permissions and carry a button to each.
