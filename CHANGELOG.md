...
- Sessions started by programs — the Agent SDK, `claude -p` from a script, a test suite — no longer fill the panel, the widget and the ready sound. They are tracked but hidden; the full panel's summary bar counts them ("3 sessions · 6 hidden", with the explanation on hover), and Settings › General › Monitoring › Show automated sessions brings them back as ordinary sessions. Interactive sessions, including `claude -p` typed into a terminal and sessions run from the Claude desktop app, are unaffected.
...
- **The permission button opens System Settings instead of putting a second prompt behind it.** Clyde asks macOS for the permission once, at launch — that ask is what creates its row in the list. The advisory's button was asking again, and the system's prompt appeared *behind* the System Settings window it had just opened, so the button looked like it did nothing.
- **Clyde offers to move itself into Applications when it is running somewhere it cannot keep a permission.** macOS keeps a separate permission for every copy of an app, and one opened straight from a disk image is copied to a temporary folder that is thrown away — so a permission granted there is lost the moment it is given, while System Settings still shows it as granted. Clyde now spots that at launch and offers to move, once. A copy sitting somewhere ordinary, like your Downloads folder, is left alone and simply mentioned in the panel if a permission is missing.
