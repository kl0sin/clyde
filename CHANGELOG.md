...
- **Worktree sessions are labelled.** A session working inside a git worktree gets a green badge with the worktree's name, instead of showing an opaque `.claude/worktrees/…` path. The badge drops if the session leaves the worktree.
