...
- **Granting the shortcut's permission now works without restarting Clyde.** The keyboard monitor binds its access when it is installed, so one created before macOS trusted Clyde kept receiving nothing afterwards — the permission read as granted in System Settings and the shortcut stayed dead, with a restart the only cure. Clyde notices the grant and rebuilds the monitor within about fifteen seconds.
- **Clyde asks macOS for its permissions instead of assuming a row exists.** Neither the Accessibility nor the Input Monitoring pane lists an application that never asked, and Clyde only ever checked — a check registers nothing. Anyone whose entry was missing or stale opened the pane to find no Clyde in it and no way to add one. Clyde now asks, which is what puts the row there.
- **The Input Monitoring button now leads somewhere you can act.** That pane lists only applications that have asked macOS for the permission, and Clyde only ever checked it — a check registers nothing. Anyone who had never been asked, or who had removed Clyde's entry, opened the pane to find no Clyde in it and no way to add one, with the shortcut stuck off and no way forward. Clyde now asks before opening the pane, which is what puts the row there.
- **The permission setting says whether it is working, not just whether it is on.** A switch reports what was asked for; it cannot tell you that Clyde's hook is not delivering, or that your sessions simply never ask. The line under it now names the actual state — off and where the questions go instead, a hook that cannot deliver, or on with nothing having arrived yet, which names the mode Claude asks in. Once a question does come through, it says when the last one was.
- **The shortcut says it needs two permissions, shows which one you have, and opens either.** ⌃⌘C needs accessibility *and* input monitoring, and macOS puts them in different panes. The advisory named only the one that was missing, so granting it produced a second warning about a permission nobody had mentioned — and the input-monitoring advisory had no button at all, leaving the user to find that pane themselves. Both advisories now name both permissions and carry a button to each — with a tick on the one that is already granted, so the finished half stops looking like work. The advisory clears itself once both are given.
