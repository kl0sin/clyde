...
- **`TeammateIdle`.** *Done 2026-08-25 (hook v29), but unverified against a real agent team.* The event addresses its target by `agent_id`, which is exactly the key space the subagent records moved into, so it annotates an existing record in place rather than needing a store of its own — and it never creates one, because a teammate we never saw start would become a phantom row. It is deliberately kept out of the attention pipeline: the item originally said it "maps onto the existing attention indicator", and that is precisely the reasoning that produced v0.5.1's false-positive badge and v0.5.2's revert a day later. Idle is a quiet row state (settled sprite, no ticking duration, grey accent) until someone observes the event's real frequency. This machine cannot produce it — no agent teams exist here, established by registering a payload dump with `Stop` as a positive control: `Stop` captured twice with a full payload, `TeammateIdle` zero times, so the negative result is the environment and not a broken probe. Agent teams were entirely invisible to Clyde before this. "A teammate is about to go idle" is the same class of signal Clyde was built around — someone is waiting on you — so it maps onto the existing attention indicator rather than needing new UI. Decide early whether a teammate is its own panel row or a decoration on the parent session's row; that choice drives everything else.
...
- **Worktree badge.** Derived from the session `cwd` (`<repo>/.claude/worktrees/<name>`), not from `WorktreeCreate` — that event is a *delegating* hook whose subscriber is expected to create the worktree and return its path, so subscribing to it breaks `EnterWorktree`. Its real payload carries only `name` (no `worktree_path` / `worktree_name`), and entering a worktree emits no `CwdChanged`, which is why the derivation lives in the hook rather than in `ProcessMonitor`. Visually this is the cleat capsule again, so the UI half is mostly plumbing.
